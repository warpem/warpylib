"""
Normalization for FFT-formatted tensors (full FFT, decentered).
"""
...
def norm_ft(
    ...
    """
    Normalize an FFT-formatted tensor (full FFT, decentered).
    ...
    by std (no mean subtraction).

    Parameters
    ----------
    tensor : torch.Tensor
        FFT-formatted tensor (real or complex), decentered
    dimensionality : int
    ...
    """
    from .masking import mask_sphere_ft
    ...
    if diameter > 0:
        ...
        mask_float = mask_sphere_ft(dummy, diameter=diameter, soft_edge_width=0.0)
    ...
    if is_complex:
        ...
        if mask is not None:
            ...
            std_val = stat_values.std(dim=spatial_dims, unbiased=False)
        ...
        if mask is not None:
            ...
            # No mask - simple mean and std
            mean_val = stat_values.mean(dim=spatial_dims, keepdim=True)
            std_val = stat_values.std(dim=spatial_dims, keepdim=True, unbiased=False)
